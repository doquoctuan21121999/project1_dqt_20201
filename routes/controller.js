...
router.get("/home", (req, res) => {
  if (req.query.search == undefined || req.query.search == "") {
    Bill.find({})
      .then((bill) => {
        bill.total = bill.Electricity_bill + bill.Room_bill + bill.Water_bill;
        res.render("home", { bills: bill });
      })
      .catch((err) => {
        console.log("Error: ", err);
...
router.post("/add-bill", async (req, res) => {
  try {
    let newBill = await new Bill({
      ...
      total:
        parseInt(req.body.electric) +
        parseInt(req.body.Water_bill1) +
        parseInt(req.body.Room_bill1),
      status: "Full",
      pay: req.body.pay1,
    });
    ...
    await Bill.findByIdAndUpdate(
      { _id: billId },
      {
        $set: {
          ...
          total:
            parseInt(req.body.Electricity_bill3) +
            parseInt(req.body.Water_bill3) +
            parseInt(req.body.Room_bill3),
          pay: req.body.pay3,
        },
      }
    );
